...
def generateDb(userIndex) :
    ...
    for i in  range(numTransactions) :
        ...
        transaction = random.sample(range(10),8)
...
def doFilter(c_k) :
    ...
    for key in frequencyCount :
        if frequencyCount[key] >= minSupport :
            f_k.append(sorted(c_k[key]))
    return f_k


# minimum support
minSupport = int(input("Enter the minimum support : "))
kValue = int(input("Enter the association size to create the sequence - usersDatabase: "))
kValue = kValue -1
kValue1 = int(input("Enter the association size to create the nonsequence usersDatase: " ))
kValue1 = kValue1 -1
...
for user in range(10) :
    ...
    if kValue1 < len(nonSequenceF) :
        userListnonsequenceDb.append(sorted(nonSequenceF[kValue1]))
    ...
    if kValue < len(sequenceF) :
        userListsequenceDb.append(sequenceF[kValue])
...
userFrequencyMap = {}
for transaction in userListnonsequenceDb :
    for item in transaction :
        stringValue = str(item)
        if stringValue in userFrequencyMap.keys() :
            userFrequencyMap[stringValue] += 1
        else :
            userFrequencyMap[stringValue] = 1
...
candidates = [0]
maxFrequencyCount = 0
for key in userFrequencyMap :    
    if userFrequencyMap[key] > maxFrequencyCount :
        candidates = []
        candidates.append(key)
        maxFrequencyCount = userFrequencyMap[key]
    elif userFrequencyMap[key] == maxFrequencyCount :
        candidates.append(key)

outputString = "The max non-sequence frequency set for %d associations " % (kValue1 +1)
for value in candidates :
    outputString += " %s %d \n" %(str(value),maxFrequencyCount)

print(outputString)

userFrequencyMap = {}
for transaction in userListsequenceDb :
    for item in transaction :
        stringValue = str(item)
        if stringValue in userFrequencyMap.keys() :
            userFrequencyMap[stringValue] += 1
...
for key in userFrequencyMap :    
    if userFrequencyMap[key] > maxFrequencyCount :
        candidates = []
        candidates.append(key)
        maxFrequencyCount = int(userFrequencyMap[key])
    elif userFrequencyMap[key] == maxFrequencyCount :
        candidates.append(key)

outputString = "The max sequence frequency set for %d associations " %(kValue +1)

for value in candidates :
    outputString += " %s with frequency count %d \n" %(str(value),maxFrequencyCount)
