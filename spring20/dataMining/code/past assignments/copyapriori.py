...
def perform_apriori(data, support_count):
    ...
    # single_items = data.apply(pd.value_counts).sum(axis=1).where(lambda value: value > support_count)
    single_items = (data['items'].split(",", expand=True))\
...
if __name__ == '__main__':
    table = pd.read_csv('data/transactionList.csv')
    
    start = time()
    print(perform_apriori(data=table, support_count=3))
    print(time() - start)