...
#for i in range(100) :
#    transaction = random.sample(range(0,15),10)
#    tDb.append(transaction)


#myFile = open("transactionDb.csv","w")

#for transaction in tDb :
#    myString = ''
#    for index,element in enumerate(transaction) :
...
def traverseTree(node,outputFile) :
    ...
    if len(node.children) == 0 :
        if node.frequency > 3 and len(node.path) > 3 :
